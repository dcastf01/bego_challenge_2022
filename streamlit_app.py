...
def error_submit():
    ...
    st.error('That is not correct cariño')
    st.stop()
...
hp_answer_submit=False
if 'is_winner' not in st.session_state:
    ...
    book_selected=st.selectbox("Choose the correct book to solve the challenge, don't worry if take time",hp_books_name_clean.keys())
    ...
    # df['contain']=np.where(df.sentences.str.contains('Alohomora'),True,False)
    # print(df[df['contain']==True])
    sentences_to_show=st.number_input('do you know what is the sentences?',min_value=0,max_value=df.shape[0])

    if sentences_to_show:
        df_aux=df.iloc[sentences_to_show]
        print(df_aux)
        st.dataframe(df_aux)
    else:
...
if hp_answer_submit:
    if hp_answer=='Alohomora':
        ...
        if 'is_winner' not in st.session_state:
            st.session_state.is_winner = True
    else:
            error_submit()
            
if 'is_winner' in st.session_state:
